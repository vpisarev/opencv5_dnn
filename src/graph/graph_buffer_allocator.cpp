...
{
    BufferAllocator(Net2* net_) : net(net_), netimpl(net_->impl()) {}
    ...
    void assign(const Graph& graph)
    {
        ...
        for (const auto& node: prog) {
            bool inplace = false;
            Arg reuseArg;
            const Op& op_ = node->op();

            if (!op_) continue;

            const BaseOp* op = op_.get();
            ...
            const std::string_view opname = op->name();
            ...
            if (opname == "If") {
                 ...
                 */
                const std::vector<Graph>& branches = node->subgraphs();
                CV_Assert(branches.size() == 2);

                const Graph& thenBranch = branches[0];
                const Graph& elseBranch = branches[1];
                ...
                CV_Assert(ninputs >= 2);
                CV_Assert(node->subgraphs().size() == 1);
                const Graph& body = node->subgraphs()[0];
