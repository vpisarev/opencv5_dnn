...
DataLayout DNN_LAYOUT_NCHWc_(int C);
bool isBlockLayout(DataLayout layout, int* C == nullptr);
...
struct CV_EXPORTS TensorShape
{
    ...
    template<typename _Tp> TensorShape(std::initializer_list<_Tp> shape,
                                        DataLayout layout=DNN_LAYOUT_UNKNOWN);
    static TensorShape fromArray(InputArray m, DataLayout layout=DNN_LAYOUT_UNKNOWN);
...
{
    constexpr size_t whole = (size_t)-1;
    struct Shared
    {
        Shared();
        void* ptr;
...
struct CV_EXPORTS Tensor
{
    ...
    template<typename _Tp> _Tp getScalar() const {
        _Tp value = 0;
        bool ok = getScalar(TypeTraits<_Tp>::depth, &value);
        return ok ? value : _Tp();
    }
    ...
    void fitSameDevice(const Tensor& tensor, const TensorShapeType& shapetyp);
    ...
    void dump(std::ostream& strm, int indent, int context=3, int maxsz_all=100, bool braces=true) const;
...
public:
    ...
    virtual void dump(std::ostream& strm, int indent) const;
...
{
    void dump(Net2& net, std::ostream& strm) const;
...
struct CV_EXPORTS OpArg
{
    OpArg();
    OpArg(int idx, Graph* g);
    int idx;
    Graph* g;
};
...
struct CV_EXPORTS OptimizedGraph
{
    ...
    virtual void dump(Net2& net, const PGraph& g, std::ostream& strm, int indent) const;
...
{
    Graph();
    Graph(Net2& net_, int graphidx_, std::string_view name_, bool ispattern_=false);
    bool empty() const;
    void clear();
    PGraph clone() const;
    void add(const Op& op, const std::vector<OpArg>& inputs, std::vector<OpArg>& outputs);
    int add1(const Op& op, std::initializer_list<OpArg> inputs);
    Net2* net() const;
    std::string_view name() const;
    int index() const;
    bool isPattern() const;
    void dump(Net2& net, std::ostream& strm, int indent);
...
struct CV_EXPORTS Backend
{
    virtual ~Backend();
    virtual Device* device() const = 0;
    virtual std::string_view name() const = 0;
    virtual bool supportType(int type) const = 0;
    virtual bool supportDynamicShapes() const = 0;
    virtual ShapeNType convertShapeType(const ShapeNType& st) const = 0;
...
public:
    ...
    void setTracingMode(TraceMode mode);
    ...
    OpArgInfo argInfo(int) const;
    OpArg newArg(const OpArgInfo& info) const;
    OpArg newConstArg(std::string_view name, const Tensor& t) const;
    OpArg newTempArg() const;
