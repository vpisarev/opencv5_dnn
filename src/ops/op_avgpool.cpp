...
static void avgpool2d_32f(const void* inp_, void* out_, const ConvState& cs, bool countIncludePadding)
{
                ...
                for(;;) {
                    if (nlanes == C0) {
                        for (; x0 < x1; x0++) {
                            ...
                            s0 = v_mul(s0, countIncludePadding ? vscale0 : vx_setall_f32(1.f/nitems));
                        ...
                        for (; x0 < x1; x0++) {
                            ...
                            for (int c = 0; c < C0; c += nlanes*2) {
                                ...
                                v_float32 vscale = countIncludePadding ? vscale0 : vx_setall_f32(1.f/nitems);
                                s0 = v_mul(s0, vscale);
                                s1 = v_mul(s1, vscale);
...
static void avgpool2d_16(const _Tp* inp_, _Tp* out_, const ConvState& cs, bool countIncludePadding)
{
    int nlanes_ = VTraits<v_float32>::vlanes();
    int C0_ = (int)cs.C0;
                ...
                for(;;) {
                    if (nlanes == C0) {
                        for (; x0 < x1; x0++) {
                            ...
                            s0 = v_mul(s0, vx_setall_f32(1.f/nitems));
                        ...
                        for (; x0 < x1; x0++) {
                            ...
                            for (int c = 0; c < C0; c += nlanes*2) {
                                ...
                                v_float32 vscale = vx_setall_f32(1.f/nitems);
...
class AveragePoolOpImpl : public AveragePoolOp
{
public:
    AveragePoolOpImpl(const ConvParams& params_, bool countIncludePadding_)
    {
        params = params_;
        countIncludePadding = countIncludePadding_;
    }
    virtual std::string_view name() const CV_OVERRIDE { return "MaxPool"; }
    virtual Op clone() const CV_OVERRIDE
    {
        return std::make_shared<AveragePoolOpImpl>(params, countIncludePadding);
    }
                            ...
                            bool symbolic) const CV_OVERRIDE
    {
        ...
        outshapes[0] = convInferShape(net, inpsize, params, TensorSize(), symbolic);
    }
                        ...
                        std::vector<Buffer>& tempbufs) CV_OVERRIDE
    {
        ...
        TensorSize outsize = convInferShape(net, inpsize, params);
        outputs.resize(1);
        ...
        func(inptr0, outptr0, cs, countIncludePadding);
    }
};

AveragePoolOp::~AveragePoolOp() {}

Op AveragePoolOp::create(const ConvParams& params, bool countIncludePadding)
{
    return std::make_shared<AveragePoolOpImpl>(params, countIncludePadding);
}

Arg averagePool(Graph& graph, std::string_view opname,
